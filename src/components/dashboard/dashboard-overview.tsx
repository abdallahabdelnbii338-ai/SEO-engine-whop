...
type OverviewData = {
  generations: { id: string; type: string; title: string; createdAt: string }[];
  projects: {
    id: string;
    name: string;
  ...
  return (
    <div>
      ...
      <section className="mb-10">
        <h2 className="text-xs font-medium text-zinc-500 uppercase tracking-wider mb-3">Tools</h2>
              ...
              <Link key={action.href} href={action.href}>
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-200 group-hover:text-zinc-50">{action.label}</p>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {data.generationCount > 0 && (
                  <p className="text-xs text-sky-400/60 mt-0.5 tabular-nums">{data.generationCount} total</p>
                )}
              </div>
            </div>
          </CardHeader>
            ...
            {data.generations.length === 0 ? (
              ...
              <ul className="divide-y divide-zinc-800/60 -mx-1">
                {data.generations.map((g) => (
                  <li
                    key={g.id}
                    className="px-1 py-3 first:pt-0 last:pb-0 hover:bg-zinc-800/20 rounded-md transition-colors"
                  >
                    <p className="text-sm text-zinc-200 leading-snug">{g.title}</p>
                    <div className="flex items-center gap-2 mt-1.5">
                      <Badge variant={TYPE_BADGE[g.type] ?? "outline"}>{TOOL_LABELS[g.type] ?? g.type}</Badge>
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                {data.projectCount > 0 && (
                  <p className="text-xs text-teal-400/60 mt-0.5 tabular-nums">{data.projectCount} total</p>
                )}
              </div>
            </div>
